...
actorOpts = rlOptimizerOptions('LearnRate', 0.001,'L2RegularizationFactor',0.0005,GradientThreshold=2,GradientThresholdMethod="l2norm"); 
criticOpts = rlOptimizerOptions('LearnRate', 0.01,'L2RegularizationFactor',0.0005,GradientThreshold=2,GradientThresholdMethod="l2norm"); 
...
for epi = 1:numOfEpisodes 
    ...
    [solution,MRHistory]=TrainPGdiscrete_solveMyProblem( problem,guess,options,trainingElemsobj,env);  

    episodeElements = trainingElemsobj.episodeOffset + (1:maxStepsPerEpisode);
    episodeCumulativeReward = extractdata(sum(trainingElemsobj.rewardBuffer(episodeElements))); 
    
    returnBuffer(episodeElements) = trainingElemsobj.rewardBuffer(episodeElements)*discountWeights;
    if mod(epi,trajectoriesForLearning) == 0

        % Reducing entropy as training progresses
        if epi > 1
            
            entropyWeight = entropyWeight * (1 - 1.5 * epi / 1000);
        end 

        if entropyWeight <= 1e-3 
            entropyWeight = 1e-3; 
        end
...
function actorGradient = actorLossFunction(actor, critic, trainingElems, returnBuffer, entropyWeight)
    ...
    pgLoss = squeeze(-sum(advantage .* actionLogProbabilities, [1,2]));
    pgLoss = pgLoss(pgLoss ~= 0);
    pgLoss = mean(pgLoss); 
    ...
    loss = (pgLoss + entropyWeight * entropyLoss) / sum(trainingElems.maskBuffer);
